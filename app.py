"""
Solar Energy Justice Index (SEJI) — v3.0
New: Optimal Location Finder, Investment Simulator, Side-by-Side Comparison,
     NTL Vulnerability Layer, Time-Series Forecasting, PDF Report Generator
"""
...
.result-highlight {{
    background: linear-gradient(135deg,rgba(0,196,180,0.08),rgba(0,196,180,0.03));
    border:1px solid rgba(0,196,180,0.3); border-radius:14px; padding:1.2rem;
}}
</style>
...
with st.sidebar:
    ...
    <div style='text-align:center;padding:0.8rem 0 0.4rem;'>
        ...
        <div style='font-size:0.62rem;color:#5A7A9A;font-family:Space Mono;
                    margin-top:2px;'>v3.0 · Indonesia 3T Regions</div>
    </div>
...
with st.spinner("Memuat..."):
    ...
    elif "Optimal Location" in page:
        from src import location_finder; location_finder.show()
    elif "SEJI Analysis" in page:
        from src import seji_analysis; seji_analysis.show()
    elif "Investment Simulator" in page:
        from src import calculator; calculator.show()
    elif "Komparasi" in page:
        from src import comparison; comparison.show()
    elif "NTL" in page:
        from src import ntl_layer; ntl_layer.show()
    elif "Time-Series" in page:
        from src import timeseries; timeseries.show()
    elif "Data Explorer" in page:
        from src import data_explorer; data_explorer.show()
    elif "Report Generator" in page:
        from src import report_generator; report_generator.show()
